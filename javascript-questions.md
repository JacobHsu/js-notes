...
通过方法 `isNaN`，`isNaN` 將參數先嘗試轉為數字，然後檢查結果是否為 `NaN`。你可以检测你传递的值是否一个 number。`name` 不是一个 `number`，將字串 "Lydia Hallie" 轉換為數字時結果是 `NaN`，因此 `isNaN(name)` 返回 `true`. `age` 是一个 `number` 因此 `isNaN(age)` 返回 `false`.
